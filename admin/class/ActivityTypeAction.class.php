 ...
 * Time: 14:12
 */
class ActivityTypeAction extends Base
{
    public function Index()
    {
        ...
        include('./template/type/activitytype.html');
    }
    public function AddSave()
    {
       $_POST['Type_Time']=time();
       //print_r($_POST);
       $this->db->Add(DBPREFIX.'activitytype',$_POST);
       echo common::MsgShow(1,'添加信息成功','./?Action=ActivityType');
    }
    public function EditSave()
    {
        ...
        if(is_array($CommonID))
        {
            foreach($CommonID as $key => $value)
            {
                ...
                $this->db->EditSave(DBPREFIX.'activitytype',$TemperArr,$Where);
            }
            echo Common::MsgShow(1,'当前信息更新修改成功','./?Action=ActivityType');
            }
    }
    public function Del()
    {
...
//            $this->db->Del(DBPREFIX.'activitytype','Type_ID in("'.$Where.'")',false);
//            echo common::MsgShow(1,'删除成功','./?Action=ActivityType');
//        }
        // 方法二：
        $Del_ID=isset($_POST['Del_ID'])?$_POST['Del_ID']:'';
        $Where=implode(",",$Del_ID);
